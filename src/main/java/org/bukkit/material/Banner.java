...
public class Banner extends MaterialData implements Attachable {
    ...
    public Banner() {
        super(BlockTypes.WHITE_BANNER.get().getDefaultState());
    }
    ...
    public @NotNull BlockFace getAttachedFace() {
        return Bonge.getInstance().convert(this.spongeValue.get(Keys.DIRECTION).get().getOpposite());
    }

    @Override
    public void setFacingDirection(@NotNull BlockFace face) {
        this.spongeValue = this.spongeValue.with(Keys.DIRECTION, Bonge.getInstance().convert(face).getOpposite()).get();
    }
